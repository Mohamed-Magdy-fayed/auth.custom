...
import type { OrganizationMembership } from "@/auth/tables/organization-memberships-table";
import { OrganizationMembershipsTable } from "@/auth/tables/organization-memberships-table";
import type { Organization } from "@/auth/tables/organizations-table";
...
export async function upsertUserOrganizations(
	...
	const organizationIdsList: string[] = organizationIds;
	...
	const toAdd: string[] = organizationIdsList.filter(
		(id) => !existingIds.includes(id),
	);
	const toRemove: string[] = existingIds.filter(
		(id) => !organizationIdsList.includes(id),
	);
...
export async function listOrganizationsForUser(userId?: string) {
	...
	const memberships = (await db.query.OrganizationMembershipsTable.findMany({
		...
		Pick<OrganizationMembership, "isDefault" | "status"> & {
			organization: Pick<
				Organization,
