				...
				<CardContent className="space-y-4 text-sm text-muted-foreground">
					{onCommunityPlan ? (
						<p>
							...
							stack is unlocked — upgrade to Plus only when you want to fund continued
							development.
						</p>
					) : (
						<>
				...
				<Card>
					...
					<CardContent className="space-y-4">
						{pendingInvites.length > 0 ? (
							<ul className="space-y-3 text-sm text-muted-foreground">
								{pendingInvites.map((invite) => (
									...
									>
										...
										<p>
											Role: {formatRole(invite.roleKey)} · Invited{" "}
							...
							<Muted>
								Looks like everyone has joined. Send more invites from the workspace
								menu.
							</Muted>
						)}
					</CardContent>
						...
						<ul className="space-y-3 text-sm text-muted-foreground">
							{recentActivity.map((event) => (
								<li key={event.id} className="rounded-lg border p-4">
									...
									<p className="mt-1 text-xs">{formatRelativeDate(event.timestamp)}</p>
								</li>
							))}
						</ul>
					) : (
						<Muted>
							No activity recorded yet. Actions like sign-ins, invites, and profile
