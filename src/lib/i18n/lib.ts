// ====================================================================================
// 1. DEFINE TRANSLATION (dt) and its supporting types
// This section is restored from your original, powerful design.
// ====================================================================================

export const LOCALE_COOKIE_NAME = "_AUTH_LOCALE";
...
// ====================================================================================
// 2. CORE i18n ENGINE
// This is the new, stable, and non-recursive type system.
// ====================================================================================
...
// Instead of intersecting the language objects, we create an ArgsMap for EACH language
// and then merge those maps. This preserves all the type information correctly.
type MergedArgsMap<T extends Record<string, LanguageMessages>> = {
	[K in keyof T]: ArgsMap<T[K]>;
}[keyof T];

// The `t` function is now typed against this correctly merged map.
...
export function createI18n<const T extends Record<string, LanguageMessages>>(
	...
	// The runtime implementation is correct and does not need to change.
	const t = (key: string, args?: Record<string, any>): string => {
		// ... runtime logic remains the same
	...
	// The cast now uses the new, correct TFunction type.
	return { t: t as TFunction<T> };
}

// ====================================================================================
// 3. RUNTIME HELPERS
// These functions perform the actual translation work at runtime.
// ====================================================================================
	...
	return Object.entries(args).reduce((result, [argKey, argValue]) => {
		...
		switch (argType) {
			...
			case "enum": {
				...
				if (replacement == null)
					throw new Error(`Missing replacement value for enum key "${argValue}"`);
			...
			case "number": {
				...
				const numberFormat = new Intl.NumberFormat(locale, params.number?.[argKey]);
			...
			case "date": {
				...
				const dateFormat = new Intl.DateTimeFormat(locale, params.date?.[argKey]);
